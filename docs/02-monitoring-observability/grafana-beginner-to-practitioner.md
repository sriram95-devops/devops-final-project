...
az group create \
  --name grafana-monitoring-rg \
  --location eastus

# Create Monitoring VM (Grafana + Prometheus run here)
az vm create \
  --resource-group grafana-monitoring-rg \
...
# Create App VM (your application runs here)
az vm create \
  --resource-group grafana-monitoring-rg \
  --name app-server-01 \
  --image Ubuntu2204 \
...
# Open required ports on Monitoring VM
az vm open-port --resource-group grafana-monitoring-rg --name monitoring-server --port 3000  # Grafana
az vm open-port --resource-group grafana-monitoring-rg --name monitoring-server --port 9090  # Prometheus
az vm open-port --resource-group grafana-monitoring-rg --name monitoring-server --port 9093  # Alertmanager

# Open Node Exporter port on App VM (restrict to monitoring VM IP in production)
az vm open-port --resource-group grafana-monitoring-rg --name app-server-01 --port 9100

# Get IPs
MONITORING_IP=$(az vm show -d -g grafana-monitoring-rg -n monitoring-server --query publicIps -o tsv)
APP_IP=$(az vm show -d -g grafana-monitoring-rg -n app-server-01 --query publicIps -o tsv)
APP_PRIVATE_IP=$(az vm show -d -g grafana-monitoring-rg -n app-server-01 --query privateIps -o tsv)
...
> Run these commands on the **app-server-01** VM (SSH into it).

```bash
# SSH into the App VM
ssh azureuser@$APP_IP

# ---- Run the following on the App VM ----

# Download Node Exporter (latest stable)
...
# Create system user (security best practice: run as non-root)
...
> Run these commands on the **monitoring-server** VM.

```bash
# SSH into the Monitoring VM
ssh azureuser@$MONITORING_IP

# ---- Run the following on the Monitoring VM ----
...
# IMPORTANT: Replace <APP_PRIVATE_IP> with the private IP of your App VM
...
scrape_configs:
  ...
  # Scrape the Monitoring VM itself (node_exporter on localhost)
  ...
  # Scrape App VM via Node Exporter
  - job_name: 'app-server-01'
    static_configs:
  ...
  # To add more VMs, copy the above job_name block and change the IP/name
EOF
